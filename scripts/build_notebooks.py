...
LAPTOP = [
    ...
    md(
        "## 1. Acquire VisA",
        "",
        "1.93 GB over HTTPS, no account. The fetch records a SHA-256 manifest so a",
    ...
    code(
        ...
        "",
        "# Tier 0 runs at 256 because these are *floors*, not candidates: their job is",
        "# to show what a trivial method achieves, and a floor measured at the study",
        "# resolution would cost far more for no extra information.",
        "TIER0_RESOLUTION = 256",
        "SEEDS = [0, 1, 2]",
        "transform = ImageTransform(mode='aspect_preserving', long_side=TIER0_RESOLUTION)",
        "",
        "tier0 = []",
        "for cat in CATEGORIES:",
    ...
    md(
        ...
        "",
        "### Choosing the resolution — and which resource actually binds",
        "",
        "Measured on this card (WideResNet50-2, `layer2+layer3`), a **native-resolution",
        "forward pass costs under 1 GB of VRAM at batch 1**. VRAM is not the constraint.",
        "",
        "What binds is the **patch matrix in host RAM**. A `layer2` grid at 1404×1070",
        "yields 23,584 patches per image, so 768 training images produce 18.1 M patches —",
        "37 GB in fp16, beyond this laptop and a Kaggle session alike. The cell below",
        "computes the largest resolution whose patch matrix fits the RAM actually free,",
        "rather than assuming one.",
        "",
        "The consequence is that the laptop PatchCore run is **knowingly handicapped**: at",
        "a reduced resolution, some fraction of defect regions is already below one pixel,",
        "and §5's table says exactly what fraction. Native-resolution PatchCore needs",
        "per-image patch subsampling *during* extraction, which does not exist yet.",
    ),
    code(
        "import psutil",
        "",
        "available_gb = psutil.virtual_memory().available / 1e9",
        "budget_gb = available_gb * 0.45      # leave room for the OS and the scoring pass",
        "n_train = max(len(indices[c]['train']) for c in CATEGORIES)",
        "",
        "print(f'host RAM available : {available_gb:.1f} GB   budget {budget_gb:.1f} GB')",
        "print(f'largest train split: {n_train} images')",
        "print()",
        "print(f\"{'long side':>10}{'patches/img':>13}{'total':>14}{'fp16 GB':>10}  fits?\")",
        "chosen = None",
        "for long_side in (256, 320, 448, 512, 704):",
        "    w, h = long_side, round(long_side * 1070 / 1404)",
        "    per_img = (w // 8) * (h // 8)",
        "    total = per_img * n_train",
        "    gb = total * 1024 * 2 / 1e9",
        "    fits = gb < budget_gb",
        "    if fits:",
        "        chosen = long_side",
        "    print(f'{long_side:>10}{per_img:>13,}{total:>14,}{gb:>10.1f}  {\"yes\" if fits else \"NO\"}')",
        "",
        "RESOLUTION = chosen or 256",
        "print(f'\\n-> PatchCore resolution: {RESOLUTION}')",
        "print('   Check this against the section-5 region-loss table: it says how many')",
        "print('   defect regions this resolution puts below one pixel.')",
    ),
    code(
        "import time, torch",
        "",
        "pc_transform = ImageTransform(mode='aspect_preserving', long_side=RESOLUTION)",
        "laptop_runs = []",
        "for cat in CATEGORIES:",
        "    seed_everything(0)",
        "    if torch.cuda.is_available():",
        "        torch.cuda.reset_peak_memory_stats()",
        "    model = build_model('patchcore', pc_transform, seed=0,",
        ...
        "    print(res.summary(), f'  [{time.perf_counter() - t0:.0f}s]')",
    ...
    code(
        "cat = CATEGORIES[0]",
        "seed_everything(0)",
        "model = build_model('patchcore', pc_transform, seed=0, backbone='wide_resnet50_2',",
...
KAGGLE = [
    ...
    md(
        ...
        "",
        "Severity is defined relative to each category's **measured median defect",
        "diameter**, so severity 3 means the same physical thing on `pcb1` as on",
        "`capsules`. ImageNet-C's constants do not transfer — its images are far smaller.",
    ),
    code(
        ...
        "from inspector.data.transforms import load_image, load_mask",
        "from inspector.metrics import compute_image_metrics, pro_curve",
        "from inspector.postproc import from_validation_percentile",
        "",
        "print('corruptions :', MAIN_SUITE)",
        "print('severities  :', SEVERITIES)",
        "print('grid        :', len(MAIN_SUITE) * len(SEVERITIES), 'cells per model per category')",
        "",
        "# The scale reference: median defect diameter, per category, in native pixels.",
        "d_med = {}",
        "for cat in CATEGORIES:",
        "    stats = summarize_regions(measure_regions(indices[cat]['test'], with_contrast=False))",
        "    d_med[cat] = stats['diameter_px_median']",
        "    print(f'{cat:<12} median defect diameter {d_med[cat]:.1f} px')",
    ),
    code(
        "# Fit once on clean data, freeze the threshold, then corrupt only the test set.",
        "# Re-thresholding under corruption would answer a much easier question.",
        "import numpy as np",
        "from inspector.data.core import ANOMALOUS",
        "",
        "def robustness_row(model, cat, corruption, severity):",
        "    idx = indices[cat]['test']",
        "    scores, labels, maps, masks = [], [], [], []",
        "    for sample in idx:",
        "        image = load_image(sample.image_path)",
        "        mask = load_mask(sample.mask_path) if sample.mask_path else None",
        "        image, mask = apply_corruption(image, mask, corruption, severity,",
        "                                       median_defect_diameter=d_med[cat], seed=0)",
        "        pred = model.predict_image(image)",
        "        scores.append(pred.score); labels.append(sample.label)",
        "        if sample.label == ANOMALOUS and mask is not None:",
        "            maps.append(pred.anomaly_map); masks.append(mask)",
        "    return np.array(scores), np.array(labels), maps, masks",
    ),
    code(
        "ROBUST_MODELS = {'patchcore': REFERENCE}",
        "robust_rows = []",
        "",
        "for cat in CATEGORIES:",
        "    for mname, cfg in ROBUST_MODELS.items():",
        "        seed_everything(0)",
        "        transform = ImageTransform(mode='aspect_preserving', long_side=RESOLUTION)",
        "        model = build_model(mname, transform, seed=0, **cfg)",
        "        model.fit(indices[cat]['train'])",
        "",
        "        # Threshold frozen on clean validation, then never touched again.",
        "        val_scores = model.image_scores(indices[cat]['validation'])",
        "        threshold = from_validation_percentile(val_scores, percentile=99.0)",
        "",
        "        for corruption in MAIN_SUITE:",
        "            for severity in (0, *SEVERITIES):",
        "                s, y, maps, masks = robustness_row(model, cat, corruption, severity)",
        "                im = compute_image_metrics(y, s, threshold=threshold.value,",
        "                                           threshold_source='validation')",
        "                aupro = pro_curve(maps, masks, integration_limit=0.05).au_pro if maps else float('nan')",
        "                robust_rows.append(dict(category=cat, model=mname, corruption=corruption,",
        "                                        severity=severity, image_auroc=im.auroc,",
        "                                        aupro_005=aupro, fpr=im.fpr, recall=im.recall))",
        "            print(f'  {cat} {mname} {corruption} done')",
        "",
        "import pandas as pd",
        "robust = pd.DataFrame(robust_rows)",
        "robust.to_csv(WORK / 'robustness.csv', index=False)",
        "print(robust.shape)",
    ),
    code(
        "# The deployment-critical view: what happens to the alarm rate at a frozen",
        "# threshold. A model can hold AUROC while its false-alarm rate triples.",
        "clean = robust[robust.severity == 0].set_index(['category', 'model', 'corruption'])",
        "",
        "print(f\"{'corruption':<18}{'sev':>4}{'d AUROC':>10}{'FPR':>9}{'FPR x clean':>13}{'recall':>9}\")",
        "print('-' * 64)",
        "for corruption in MAIN_SUITE:",
        "    for sev in SEVERITIES:",
        "        rows = robust[(robust.corruption == corruption) & (robust.severity == sev)]",
        "        base = robust[(robust.corruption == corruption) & (robust.severity == 0)]",
        "        d_auc = rows.image_auroc.mean() - base.image_auroc.mean()",
        "        ratio = rows.fpr.mean() / max(base.fpr.mean(), 1e-6)",
        "        print(f'{corruption:<18}{sev:>4}{d_auc:>+10.4f}{rows.fpr.mean():>9.3f}'",
        "              f'{ratio:>13.1f}x{rows.recall.mean():>9.3f}')",
    ),
    code(
        "# 'Severity at which it breaks': the lowest severity where the realized",
        "# false-alarm rate exceeds 3x its clean value. More actionable than an",
        "# area-under-degradation number, because it converts to a maintenance tolerance.",
        "print(f\"{'corruption':<18}{'breaks at severity'}\")",
        "print('-' * 40)",
        "for corruption in MAIN_SUITE:",
        "    base = robust[(robust.corruption == corruption) & (robust.severity == 0)].fpr.mean()",
        "    broke = None",
        "    for sev in SEVERITIES:",
        "        f = robust[(robust.corruption == corruption) & (robust.severity == sev)].fpr.mean()",
        "        if f > 3 * max(base, 1e-6):",
        "            broke = sev",
        "            break",
        "    print(f'{corruption:<18}{broke if broke else \"survives all 5\"}')",
    ),
    md(
        "### The validity question",
        "",
        "Synthetic illumination corruption is a *model* of real lighting shift. Whether",
        "it predicts real behaviour is an empirical question, and it can only be answered",
        "on MVTec AD 2, whose `test_private_mixed` split is captured under genuinely",
        "unseen lighting.",
        "",
        "When AD 2 is available, compute the Spearman rank correlation between the model",
        "ranking under `exposure_*` / `spatial_light` and the ranking under the real",
        "shift. High correlation validates synthetic robustness testing as a cheap proxy;",
        "low correlation is a finding about a practice the field relies on. Either result",
        "is worth writing up — it is the highest-value hour in Phase P7.",
    ),
    md(
